<?php /* GrowMate: Focus Timer + Anti-Distraksi */ ?>
<x-app-layout>
    <style>
        [x-cloak] {
    ...
    <div class="growmate-container pt-6 pb-20"
        ...
        data-store-url=" route('timer.store') "
        data-csrf=" csrf_token() ">

       
        <div x-show="showLock" x-cloak
            class="fixed inset-0 z-50 flex items-center justify-center bg-[#2b3650]/85 backdrop-blur-sm">
            <div class="bg-white rounded-2xl p-8 max-w-sm text-center shadow-2xl">
                <div class="text-5xl mb-2">🔒</div>
                <h2 class="text-xl font-bold text-gray-800 mb-2">Eitss, fokus dulu!</h2>
                <p class="text-sm text-soft-text mb-1">Kamu sempat meninggalkan halaman saat sesi fokus jalan, jadi timernya otomatis dijeda.</p>
                <p class="text-sm mb-4">Teralihkan <span class="font-bold text-red-500" x-text="distraksi"></span>x di sesi ini.</p>
                <button @click="lanjutFokus()"
                    class="bg-gradient-to-r from-primary to-primary-dark text-white font-medium px-6 py-3 rounded-xl hover:shadow-lg transition-all active:scale-95">
                    Lanjut Fokus
                </button>
            </div>
        </div>
    ...
    <script>
        function focusTimer() {
            return {
                ...
                init() {
                    // ambil nilai dari atribut data-* di div utama
                    this.sessionCount = parseInt(this.$el.dataset.todayCount) || 0;
                    this.storeUrl = this.$el.dataset.storeUrl;
                    this.csrf = this.$el.dataset.csrf;

                    // anti-distraksi: deteksi pindah tab pas sesi fokus jalan
                    document.addEventListener('visibilitychange', () => {
                        if (document.hidden && this.status === 'running' && this.mode === 'focus') {
                            this.distraksi++;
                            this.pauseTimer();
                            document.title = 'Balik ke fokus!';
                        } else if (!document.hidden && this.status === 'paused' && this.distraksi > 0) {
                            this.showLock = true;
                ...
                startTimer() {
                    ...
                    if (this.status === 'idle') this.distraksi = 0;
                    this.status = 'running';
